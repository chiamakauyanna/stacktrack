          ...
          <div className="bg-red-50 p-4 rounded-2xl shadow-md hover:shadow-xl transition-transform hover:-translate-y-1">
            ...
            <div className="space-y-2">
              {overdue.map(task => (
                ...
                >
                  <div>
                    <h4 className="text-sm font-medium text-red-700">{task.title || "Untitled Task"}</h4>
                    {task.due_date && (
                      <p className="text-xs text-gray-600">Due: {dayjs(task.due_date).format("MMM D, YYYY")}</p>
                    )}
                  </div>
          ...
          <div className="bg-yellow-50 p-4 rounded-2xl shadow-md hover:shadow-xl transition-transform hover:-translate-y-1">
            ...
            <div className="space-y-2">
              {dueSoon.map(task => (
                ...
                >
                  <div>
                    <h4 className="text-sm font-medium text-yellow-800">{task.title || "Untitled Task"}</h4>
                    {task.due_date && (
                      <p className="text-xs text-gray-600">Due: {dayjs(task.due_date).format("MMM D, YYYY")}</p>
                    )}
                  </div>
